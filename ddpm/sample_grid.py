"""Sample a batch of RSICD test-set captions through a trained cascade.
...
def build_imagen(img_sz: int, ts: int) -> tuple[Imagen, Unet]:
    """Mirror ddpm/models/Imagen_text_pytorch.py build_models()."""
    unet = Unet(
        dim=128,
        cond_dim=256,
        dim_mults=(1, 2, 2, 2),
...
def build_imagen_sr(img_sz: int, sr_sz: int, ts: int) -> Imagen:
    """Mirror ddpm/models/Imagen_text_sr_pytorch.py build_models() (path B).
