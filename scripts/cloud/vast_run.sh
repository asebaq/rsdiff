...
#   bootstrap             Run scripts/cloud/vast_setup.sh --download-rsicd remotely.
#   run [EPOCHS] [NAME]   Launch LR training (ddpm/scripts/run_smoke.sh) via nohup; survives disconnect.
#                         Default: 10 epochs, LOG_NAME=smoke_lr_gdm. Use 1000 + full_lr_gdm for full run.
#   run-sr [EPOCHS] [NAME]  Launch SR training (ddpm/scripts/run_sr.sh, path B) via nohup.
#   run-joint [EPOCHS] [NAME]  Launch joint fine-tune (ddpm/scripts/run_joint.sh, both unets) via nohup.
#                         Default: 1000 epochs, LOG_NAME=full_sr_gdm. Seeds+freezes LR base.
#                         Env: LR_CKPT overrides the base checkpoint to seed unet 1.
#   logs [NAME]           Tail logfile.log from ddpm/logs/<NAME>/ (default smoke_lr_gdm).
...
#                         Default SUBPATH=ddpm/logs/smoke_lr_gdm.
...
#   pull-milestones [NAME]  Rsync ddpm/logs/<NAME>/milestones/ -> outputs/vast/.../milestones/.
...
#   VAST_STATE=fid ./scripts/cloud/vast_run.sh launch ...  -> .vast/fid.json
...
cmd_bootstrap() {
  require_state
  ssh_cmd "cd ${REMOTE_ROOT} && bash scripts/cloud/vast_setup.sh --download-rsicd"
}
...
cmd_run() {
  ...
  ssh_cmd "mkdir -p ${REMOTE_ROOT}/outputs && cd ${REMOTE_ROOT} && \
    EPOCHS=${epochs} LOG_NAME=${name} nohup bash ddpm/scripts/run_smoke.sh \
    ...
    echo started run pid=\$! name=${name} epochs=${epochs} \
      ...
      logger_log=ddpm/logs/${name}/logfile.log"
}
...
cmd_run_sr() {
  ...
  local ts="$(date +%Y%m%d_%H%M%S)"
  ssh_cmd "mkdir -p ${REMOTE_ROOT}/outputs && cd ${REMOTE_ROOT} && \
    EPOCHS=${epochs} LOG_NAME=${name} ${LR_CKPT:+LR_CKPT='${LR_CKPT}'} nohup bash ddpm/scripts/run_sr.sh \
      > ${REMOTE_ROOT}/outputs/run_${name}_${ts}.log 2>&1 < /dev/null & \
...
cmd_run_joint() {
  ...
  local ts="$(date +%Y%m%d_%H%M%S)"
  ssh_cmd "mkdir -p ${REMOTE_ROOT}/outputs && cd ${REMOTE_ROOT} && \
    EPOCHS=${epochs} LOG_NAME=${name} ${INIT_CKPT:+INIT_CKPT='${INIT_CKPT}'} ${LAMBDA_SR:+LAMBDA_SR='${LAMBDA_SR}'} nohup bash ddpm/scripts/run_joint.sh \
      > ${REMOTE_ROOT}/outputs/run_${name}_${ts}.log 2>&1 < /dev/null & \
...
cmd_logs() {
  ...
  ssh_cmd "tail -n 60 ${REMOTE_ROOT}/ddpm/logs/${name}/logfile.log"
}
...
cmd_snapshot() {
  ...
  local logdir="${REMOTE_ROOT}/ddpm/logs/${name}"
  if [ "${watch}" -eq 0 ]; then
    ssh_cmd "bash ${REMOTE_ROOT}/scripts/cloud/remote_snapshot.sh '${logdir}' '${max}' '${prune_to}'"
  else
    ssh_cmd "nohup bash -c 'while true; do bash ${REMOTE_ROOT}/scripts/cloud/remote_snapshot.sh \"${logdir}\" \"${max}\" \"${prune_to}\" 2>&1; sleep ${watch}; done' > ${REMOTE_ROOT}/outputs/snap_watcher_${name}.log 2>&1 < /dev/null & echo started watcher pid=\$! interval=${watch}s max=${max} prune_to=${prune_to} log=outputs/snap_watcher_${name}.log"
  fi
}

cmd_pull() {
  require_state
  local sub="${1:-ddpm/logs/smoke_lr_gdm}"
...
cmd_sample_grid() {
  ...
  ssh_cmd "cd ${REMOTE_ROOT} && python ddpm/sample_grid.py \
    --log_dir ${logdir} \
...
cmd_pull_milestones() {
  ...
  host="$(state_get ssh_host)"
  port="$(state_get ssh_port)"
  dest="${REPO_ROOT}/outputs/vast/ddpm/logs/${name}/milestones"
  remote_dir="${REMOTE_ROOT}/ddpm/logs/${name}/milestones"
  mkdir -p "${dest}"
