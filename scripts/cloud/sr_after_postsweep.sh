...
cd "$ROOT"
WINNER_STEP="$winner_step" bash scripts/eval/sr_cfg_sweep.sh 2>&1 | tee "$ROOT/cfgsweep.log"
...
WINNER_STEP="$winner_step" CFG_SCALE="$winner_scale" bash scripts/eval/sr_final_1093.sh 2>&1 | tee "$ROOT/final1093.log"
...
final_dir="$ROOT/ddpm/logs/full_sr_gdm/generated_images/final_test_step${winner_step}_cs${winner_scale}"
echo "=== CLIP score start $(date -u) ==="
GEN_DIR="$final_dir" bash scripts/eval/sr_clip_score.sh 2>&1 | tee "$ROOT/clipscore.log"
