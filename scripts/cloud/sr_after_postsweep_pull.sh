...
$SSH "$HOST" "ls -d $BR/ddpm/logs/full_sr_gdm/generated_images/cfg*_step*_cs* 2>/dev/null" | while read -r d; do
  base=$(basename "$d")
  rsync $RSYNC_OPTS "$SSH" "$HOST:$d/" "$LR/ddpm/logs/full_sr_gdm/generated_images/$base/"
done

# Final 1093 bundle(s)
$SSH "$HOST" "ls -d $BR/ddpm/logs/full_sr_gdm/generated_images/final_test_step* 2>/dev/null" | while read -r d; do
  base=$(basename "$d")
  rsync $RSYNC_OPTS "$SSH" "$HOST:$d/" "$LR/ddpm/logs/full_sr_gdm/generated_images/$base/"
done

# Sweep gen sample dirs (fid128_step*) — ~1.5 GB total
$SSH "$HOST" "ls -d $BR/ddpm/logs/full_sr_gdm/generated_images/fid128_step* 2>/dev/null" | while read -r d; do
  base=$(basename "$d")
  rsync $RSYNC_OPTS "$SSH" "$HOST:$d/" "$LR/ddpm/logs/full_sr_gdm/generated_images/$base/"
done
...
rsync $RSYNC_OPTS "$SSH" --include='sample-SR-UNet-*.png' --include='*/' --exclude='*' \
  "$HOST:$BR/ddpm/logs/full_sr_gdm/generated_images/" \
  "$LR/ddpm/logs/full_sr_gdm/generated_images/"
...
ls -ld "$LR/ddpm/logs/full_sr_gdm/generated_images/cfg"* \
       "$LR/ddpm/logs/full_sr_gdm/generated_images/final_test_step"* \
       "$LR/ddpm/logs/full_sr_gdm/generated_images/fid128_step"* 2>/dev/null
