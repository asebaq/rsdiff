...
#
# Pair: scripts/cloud/sr_post_sweep_pull.sh (local-side rsync watcher).
set -uo pipefail
ROOT="${ROOT:-/workspace/rsdiff}"
LOG="$ROOT/ddpm/logs/full_sr_gdm"
...
if [ -d "$LOG/generated_images/$out" ]; then
  ...
  echo "=== gen $out $(date -u) ==="
  python ddpm/sample_grid.py --log_dir "$LOG" \
    --data_root "$ROOT/ddpm/RSICD_optimal" \
