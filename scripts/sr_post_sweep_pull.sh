...
#
# Pair: scripts/sr_post_sweep.sh (box-side finalizer).
set -uo pipefail
...
rsync $RSYNC_OPTS "$SSH" "$HOST:$BR/legacy/DDPM/logs/full_sr_gdm/srsnap.log" "$LR/legacy/DDPM/logs/full_sr_gdm/"
rsync $RSYNC_OPTS "$SSH" "$HOST:$BR/legacy/DDPM/logs/full_sr_gdm/generated_images/grid_sr_ep500/" \
  "$LR/legacy/DDPM/logs/full_sr_gdm/generated_images/grid_sr_ep500/"

echo "=== ALL DONE $(date -u) ==="
ls -la "$LR/fid/fid_curve_full_sr_gdm.tsv" "$LR/fid/fidsweep.log" \
       "$LR/legacy/DDPM/logs/full_sr_gdm/srsnap.log" \
       "$LR/legacy/DDPM/logs/full_sr_gdm/generated_images/grid_sr_ep500/" 2>/dev/null
