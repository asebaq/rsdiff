...
#   1. Symlinks data/RSICD_optimal -> ddpm/RSICD_optimal (fid_sweep.sh expects data/).
...
mkdir -p "$ROOT/data"
[ -L "$ROOT/data/RSICD_optimal" ] || ln -sf "$ROOT/ddpm/RSICD_optimal" "$ROOT/data/RSICD_optimal"
...
bash scripts/eval/fid_sweep.sh 2>&1 | tee $ROOT/fidsweep.log
"
sleep 2
tmux ls
echo "--- launched ---"
